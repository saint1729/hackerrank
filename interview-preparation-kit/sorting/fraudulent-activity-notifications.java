...
public class Solution {
    ...
    static int activityNotifications(int[] expenditure, int d) {
        ...
        for(int i = d; i < expenditure.length; i++) {
            int twoMedian = window[d/2]+window[(d-1)/2];
            if(twoMedian <= expenditure[i]) {
                ans += 1;
            }
            remove(window, expenditure[i-d]);
            add(window, expenditure[i]);
        }
    ...
    static void remove(int[] window, int val) {
        int index = Arrays.binarySearch(window, val);
        if(index < 0) {
            index = -index-1;
        }
        int j = index;
        while(j < window.length-1) {
            window[j] = window[++j];
        }
        return;
    }

    static void add(int[] window, int val) {
        int index = Arrays.binarySearch(window, val);
        if(index < 0) {
            index = -index-1;
        }
